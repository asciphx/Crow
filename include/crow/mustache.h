...
namespace crow {
  namespace mustache {
	...
	inline std::string default_loader(const char* filename) {
	  std::string path = detail::directory_ + filename;
	  std::ifstream inf(path);
	  if (!inf) return {};
	  return { std::istreambuf_iterator<char>(inf), std::istreambuf_iterator<char>() };
	...
	inline template_t load(std::string& filename) {
	  return template_t(default_loader(filename));
	}

	//inline void set_loader(std::function<std::string(std::string)> loader) {
	//  get_loader_ref() = std::move(loader);
	//}
	inline template_t load(const char* filename) {
	  return template_t(default_loader(filename));
	}
  }
}
