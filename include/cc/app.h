...
#else
#define ROUTE(app, url) app[url]
#endif
#define CATCHALL_ROUTE(app) app.default_route()

namespace cc { static std::string RES_home = HOME_PAGE; static std::string_view RES_oct("application/octet-stream", 24); unsigned short detail::dumb_timer_queue::tick = 4;
#ifdef ENABLE_SSL
...
#ifndef DISABLE_HOME
#ifdef ENABLE_COMPRESSION
 route_url<cc::spell::get_parameter_tag("/")>("/")([](Req& req, Res& res) { res.compressed = false; if (RES_CACHE_TIME[0] > nowStamp()) { switch (comp_algorithm_) { case compression::GZIP: res.set_header(RES_CE, RES_gzip); break; default:res.set_header(RES_CE, RES_deflate); } return RES_CACHE_MENU[0]; } LOG_WARNING("#ENABLE_COMPRESSION" << res.compressed); std::string s = std::move((std::string)mustache::load(RES_home)); switch (comp_algorithm_) { case compression::GZIP:s = std::move(compression::compress_string(s, compression::algorithm::GZIP)); res.set_header(RES_CE, RES_gzip); break; default:s = std::move(compression::compress_string(s, compression::algorithm::DEFLATE)); res.set_header(RES_CE, RES_deflate); }
#else
 route_url<cc::spell::get_parameter_tag("/")>("/")([] { if (RES_CACHE_TIME[0] > nowStamp()) { return RES_CACHE_MENU[0]; } std::string s = std::move((std::string)mustache::load(RES_home));
#endif
 RES_CACHE_TIME[0] = nowStamp(detail::dumb_timer_queue::tick); RES_CACHE_MENU[0] = s; return s; });
#endif
 route_url<cc::spell::get_parameter_tag("/_")>("/_")([] {  return "are you ok?"; }); validate();
#ifdef ENABLE_SSL
...
#endif
  using context_t = detail::Ctx<Middlewares...>; template <typename T> typename T::Ctx& get_context(Req & req) { static_assert(spell::contains<T, Middlewares...>::value, "App doesn't have the specified middleware type."); auto& ctx = *reinterpret_cast<context_t*>(req.middleware_context); return ctx.template get<T>(); } template <typename T> T& get_middleware() { return utility::get_element_by_type<T, Middlewares...>(middlewares_); }  void wait_for_server_start() { std::unique_lock<std::mutex> lock(start_mutex_); if (server_started_) return; cv_started_.wait(lock); } private: uint16_t concurrency_ = 1; Router router_; bool is_not_set_types = true;
#ifdef ENABLE_COMPRESSION
 static inline compression::algorithm comp_algorithm_ = cc::compression::GZIP;
#endif
