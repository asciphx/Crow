...
#ifndef DISABLE_HOME
 route_url<cc::spell::get_parameter_tag("/")>("/")([](Req& req) {
#ifdef ENABLE_COMPRESSION
 req.home_need = true; if (RES_CACHE_TIME[0] > nowStamp()) { return RES_CACHE_MENU[0]; } std::string s = std::move((std::string)mustache::load(RES_home)); switch (comp_algorithm_) { case compression::GZIP:s = std::move(compression::compress_string(s, compression::algorithm::GZIP));break; default:s = std::move(compression::compress_string(s, compression::algorithm::DEFLATE)); }
#else
 if (RES_CACHE_TIME[0] > nowStamp()) { return RES_CACHE_MENU[0]; } std::string s = std::move((std::string)mustache::load(RES_home));
#endif
