...
#endif
namespace cc { using namespace boost; unsigned int utimeout_milli = 3500; using tcp=asio::ip::tcp; 
#if defined __linux__ || defined __APPLE__
 struct timeval tv { utimeout_milli / 1000, utimeout_milli };
#else
 int32_t timeout = cc::utimeout_milli; 
#endif
...
#else
 setsockopt(socket_.native_handle(),SOL_SOCKET,SO_RCVTIMEO,(const char*)&timeout,sizeof(timeout)); setsockopt(socket_.native_handle(),SOL_SOCKET,SO_SNDTIMEO,(const char*)&timeout,sizeof(timeout));
#endif
