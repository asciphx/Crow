...
namespace cc { static std::unordered_map<uint64_t, std::string> RES_CACHE_MENU = {}; static std::unordered_map<uint64_t, int64_t> RES_CACHE_TIME = {}; static tm* RES_NOW; static time_t RES_TIME_T; static std::string RES_DATE_STR; static auto RES_last = std::chrono::steady_clock::now(); tm now() { return *RES_NOW; } inline int64_t nowStamp(short& i) { return RES_TIME_T + i; } inline int64_t nowStamp(short&& i) { return RES_TIME_T + i; } inline int64_t nowStamp() { return RES_TIME_T; } namespace detail { static std::string directory_ = STATIC_DIRECTORY;  class dumb_timer_queue { public: static unsigned short tick; dumb_timer_queue(boost::asio::io_service& io_service) : io_service_(io_service), deadline_timer_(io_service_) {
#ifdef _WIN32
