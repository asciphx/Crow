...
#endif
 deadline_timer_.async_wait(std::bind(&dumb_timer_queue::tick_handler, this, std::placeholders::_1)); } ~dumb_timer_queue() { deadline_timer_.cancel(); } inline void cancel(uint16_t& id) { LOG_INFO(" -key:< " << id << " >"); dq_.erase(id); id = 0; } inline size_t add(const std::function<void()>& task) { if (step_ == 0xffff)++step_; dq_.insert({ ++step_, {std::chrono::steady_clock::now() + std::chrono::seconds(tick), task} }); std::cout << step_ << "< "; return step_; } inline size_t add(const std::function<void()>& task, std::uint8_t& timeout) { if (step_ == 0xffff)++step_; dq_.insert({ ++step_, {std::chrono::steady_clock::now() + std::chrono::seconds(timeout), task} }); return step_; } inline size_t add(const std::function<void()>& task, std::uint8_t&& timeout) { if (step_ == 0xffff)++step_; dq_.insert({ ++step_, {std::chrono::steady_clock::now() + std::chrono::seconds(timeout), task} }); return step_; } private: void tick_handler(...) { 
#ifdef _WIN32
