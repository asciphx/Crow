...
namespace cc { static const char RES_GMT[26] = "%a, %d %b %Y %H:%M:%S GMT"; static uint16_t port_ = DEFAULT_PORT; static std::string bindaddr_ = "0.0.0.0"; using namespace boost; using tcp = asio::ip::tcp; template <typename Handler, typename Adaptor = SocketAdaptor, typename ... Middlewares> class Server { public: Server(Handler* handler, std::tuple<Middlewares...>* middlewares = nullptr, uint16_t concurrency = 1, typename Adaptor::Ctx* adaptor_ctx = nullptr) : acceptor_(io_service_, tcp::endpoint(boost::asio::ip::address::from_string(cc::bindaddr_), cc::port_)), signals_(io_service_, SIGINT, SIGTERM), handler_(handler), concurrency_(concurrency < 2 ? 2 : concurrency), roundrobin_index_(concurrency_ - 1), core_(concurrency_ - 1), middlewares_(middlewares), adaptor_ctx_(adaptor_ctx) {} Server(Handler* handler, std::string addr = cc::bindaddr_, uint16_t port = cc::port_, std::tuple<Middlewares...>* middlewares = nullptr, uint16_t concurrency = 1, typename Adaptor::Ctx* adaptor_ctx = nullptr) : acceptor_(io_service_, tcp::endpoint(boost::asio::ip::address::from_string(std::move(addr)), port)), signals_(io_service_, SIGINT, SIGTERM), handler_(handler), concurrency_(concurrency < 2 ? 2 : concurrency), roundrobin_index_(concurrency_ - 1), core_(concurrency_ - 1), middlewares_(middlewares), adaptor_ctx_(adaptor_ctx) {} void run() { if (!std::filesystem::is_directory(detail::directory_)) { std::filesystem::create_directory(detail::directory_); } if (!std::filesystem::is_directory(detail::directory_ + detail::upload_path_)) { std::filesystem::create_directory(detail::directory_ + detail::upload_path_); } for (int i = 0; i < concurrency_; ++i) io_service_pool_.emplace_back(new boost::asio::io_service()); get_cached_date_str_pool_.resize(concurrency_); timer_queue_pool_.resize(concurrency_); std::vector<std::future<void>> v; std::atomic<int> init_count(0); for (uint16_t i = 0; i < concurrency_; ++i) v.push_back( std::async(std::launch::async, [this, i, &init_count] { get_cached_date_str_pool_[i] = []()->std::string { if (std::chrono::steady_clock::now() - RES_last > std::chrono::seconds(2)) { time(&RES_TIME_T); RES_last = std::chrono::steady_clock::now();
