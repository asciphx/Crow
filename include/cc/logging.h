...
namespace cc { enum class LogLevel { DEBUG=0,INFO,WARNING,ERR,CRITICAL, }; class ILogHandler { public:virtual void log(std::string message,LogLevel level)=0; }; class CerrLogHandler : public ILogHandler { public:void log(std::string message,LogLevel) override { std::cerr<<message; } }; class logger { private: static std::string timestamp() { char date[32]; time_t t=time(0); tm my_tm;
...
#define LOG_CRITICAL   if (cc::logger::get_current_log_level() <= cc::LogLevel::CRITICAL)  cc::logger("CRITICAL", cc::LogLevel::CRITICAL)

#define LOG_ERROR  if (cc::logger::get_current_log_level() <= cc::LogLevel::ERR)  cc::logger("ERROR  ", cc::LogLevel::ERR)

#define LOG_WARNING  if (cc::logger::get_current_log_level() <= cc::LogLevel::WARNING)  cc::logger("WARNING ", cc::LogLevel::WARNING)

#define LOG_INFO   if (cc::logger::get_current_log_level() <= cc::LogLevel::INFO)  cc::logger("INFO ", cc::LogLevel::INFO)

#define LOG_DEBUG  if (cc::logger::get_current_log_level() <= cc::LogLevel::DEBUG)  cc::logger("DEBUG  ", cc::LogLevel::DEBUG)
