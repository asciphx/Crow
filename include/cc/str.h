...
extern "C" {
#endif
 static char RES_ASCII[97] = " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"; static int strLen(const char* s) { const char* e = s; while (*++e); return e - s; } static void strCpy(char* d, const char* s) { while (*s) { *d++ = *s++; }*d = 0; } static char* strStr(char* d, const char* s) { int i = 0, j = 0, l = strLen(d), k = strLen(s); while (i < l&& j < k) if (d[i] == s[j])++i, ++j; else i = i - j + 1, j = 0; if (j == k) { return d + i - k; } return (char*)0; } static int strCmp(const char* c, const char* s) { while (*s == *c && *c && *s)++c, ++s; return *c == *s ? 0 : *c > *s ? 1 : -1; } static int find1CharPosDESC(const char* c, const char d) { for (int l = strLen(c), i = l - 1; i > 0; --i) { if (c[i] == d)return l - i; } return -1; } static int find1CharPosASC(const char* c, const char d) { for (int l = strLen(c), i = 0; i < l; ++i) { if (c[i] == d)return i; }return -1; }  char* subStr(const char* c, int i, int e) { if (e < i || i < 0)return (char*)0; char* w = (char*)malloc(sizeof(char) * (e - i + 1)); int p = 0; while (i < e)w[p++] = c[i++]; w[p] = 0; return w; } char* joinS2CInInt(const char* c, int i, const char* s) { int j = strLen(s), l = strLen(c), p = 0, n = 0, m = 0; char* w = (char*)malloc(sizeof(char) * (l + j)); --i; while (p < i)w[p++] = c[n++]; while (m < j)w[p++] = s[m++]; while (n < l) { w[p++] = c[n++]; } w[p] = '\0'; return w; } char* to8Str(unsigned long long i) { int z = 2; for (unsigned long long a = i; a > 0x7f; a -= 0x7f, a /= 0x100, ++z); unsigned long long b, t = i / 0x100; b = i - t * 0x100 - 32; char* w = (char*)malloc(sizeof(char) * z); w[--z] = '\0'; while (t > 0x7f) { w[--z] = RES_ASCII[b]; i = t; t = i / 0x100; b = i - t * 0x100 - 32; } w[--z] = RES_ASCII[b]; if (z > 0) { t -= 32; w[0] = RES_ASCII[t]; } return w; } char* to4Str(int i) { int t = i / 0x100, b = i - t * 0x100 - 32, z = i > 0x7f7f7f ? 5 : i > 0x7f7f ? 4 : i > 0x7f ? 3 : 2; char* w = (char*)malloc(sizeof(char) * z); w[--z] = 'a'; while (t > 0x7f) { w[--z] = RES_ASCII[b]; i = t; t = i / 0x100; b = i - t * 0x100 - 32; } w[--z] = RES_ASCII[b]; if (z > 0) { t -= 32; w[0] = RES_ASCII[t]; } return w; } static int hack_4str(const char* oid) { int t = 0, i = strLen(oid), j, pow = 1; while (i-- > 0) { char* chr = subStr(oid, i, i + 1); j = 0; while (j++ < 0x61) { char* s = subStr(RES_ASCII, j - 1, j); if (strCmp(chr, s) == 0) { t = t + (j + 0x1f) * pow; pow = pow * 0x100; free(s); break; } free(s); } free(chr); } return t; } static inline constexpr unsigned long long hack8Str(const char* s) { unsigned long long r = s[0]; for (signed char i = 0; ++i < 8 && s[i]; r *= 0x100, r += s[i]); return r; } static inline constexpr int hack4Str(const char* s) { int r = 0; for (signed char i = -1; ++i < 4 && s[i]; r *= 0x100, r += s[i]); return r; }  static inline constexpr unsigned long long hackStr(const char* s) { unsigned long long r = s[0]; for (unsigned short i = 0; s[++i]; r *= 0x1f, r += s[i] - 0x2f); return r; } static inline constexpr unsigned long long hackUrl(const char* s) { unsigned long long r = s[0]; for (unsigned char i = 0; s[++i] > 0x22; r << 4, r += s[i] > 0x5e ? s[i] - 0x5f : s[i] > 0x40 ? s[i] - 0x3f : s[i] - 3); return r; }  static inline short atos_(char* c) { short r = 0; if (*c == '-') { while (*++c) r = r * 10 - *c + 0x30; } else { while (*c) r = r * 10 + *c++ - 0x30; } return r; } static inline int atoi_(char* c) { int r = 0; if (*c == '-') { while (*++c) r = r * 10 - *c + 0x30; } else { while (*c) r = r * 10 + *c++ - 0x30; } return r; } static inline long long atol_(char* c) { long long r = 0; if (*c == '-') { while (*++c) r = r * 10 - *c + 0x30; } else { while (*c) r = r * 10 + *c++ - 0x30; } return r; } static inline long long aton_(char* c) { long long r = 0; while (*++c) r = r * 10 - *c + 0x30; return r; } static inline unsigned long long atou_(char* c) { unsigned long long r = 0; while (*c) r = r * 10 + *c++ - 0x30; return r; }
#ifdef __cplusplus
} constexpr unsigned long long operator""_l(const char* s, size_t ) { unsigned long long r = s[0]; for (unsigned long long i = 0; ++i < 8 && s[i]; r *= 0x100, r += s[i]); return r;}constexpr int operator""_i(const char* s, size_t ) { int r = s[0]; for (int i = 0; ++i < 4 && s[i]; r *= 0x100, r += s[i]); return r;}constexpr unsigned long long operator""_a(const char* s, size_t ) { unsigned long long r = s[0]; for (unsigned long long i = 1; s[i]; r *= 0x1f, r += s[i++] - 0x2f); return r;}
#ifdef _WIN32
#define _IS_WIN 1
#else
#define _IS_WIN 0
#endif
#define HEX2URL(h) if (h > 0x2f && h < 0x3a) { h -= 0x30; } else if (h > 0x60 && h < 0x67) { h -= 0x57; } else if (h > 0x40 && h < 0x47) { h -= 0x37; }

std::string DecodeURL(const std::string& c) { std::string s; size_t len = c.size(); s.reserve(len * 3 / 2); for (size_t i = 0; i < len; ++i) { switch (c[i]) { case 0x25: if (isxdigit(c[i + 1]) && isxdigit(c[i + 2])) { unsigned int h = c[++i], l = c[++i]; HEX2URL(h) HEX2URL(l) s += char((h << 4) + l); } else { s += 0x25; } break; case 0x1f: s += 0x20; break; default: s += static_cast<unsigned int>(c[i]); } } return s;}inline std::string_view UnescapeURL(std::string_view str) { char* o = (char*)str.data(); char* c = (char*)str.data(); const char* end = c + str.size(); while (c < end) { if (*c == '%' && c < end - 2) { char number[3]; number[0] = c[1]; number[1] = c[2]; number[2] = 0; *o = char(strtol(number, nullptr, 16)); c += 2; } else if (o != c) *o = *c; ++o; ++c; } return std::string_view(str.data(), o - str.data());}static std::string& toUpperCase(std::string& s) { char* c = (char*)s.c_str(); if (*c > 0x60 && *c < 0x7b) { *c &= ~0x20; } while (*++c) { if (*c > 0x60 && *c < 0x7b) *c &= ~0x20; } return s;}static std::string toUpperCase(const char* s) { std::string e; if (*s > 0x60 && *s < 0x7b) { e.push_back(*s - 0x20); } while (*++s) { if (*s > 0x60 && *s < 0x7b) { e.push_back(*s - 0x20); } else { e.push_back(*s); } } return e;}static std::string& toLowerCase(std::string& s) { char* c = (char*)s.c_str(); if (*c > 0x40 && *c < 0x5b) { *c |= 0x20; } while (*++c) { if (*c > 0x40 && *c < 0x5b) *c |= 0x20; } return s;}static std::string toLowerCase(const char* s) { std::string e; if (*s > 0x40 && *s < 0x5b) { e.push_back(*s + 0x20); } while (*++s) { if (*s > 0x40 && *s < 0x5b) { e.push_back(*s + 0x20); } else { e.push_back(*s); } } return e;}static std::string toSqlCase(const char* s) { std::string e; if (*s > 0x40 && *s < 0x5b) { e.push_back(*s + 0x20); } while (*++s) { if (*s > 0x40 && *s < 0x5b) { e.push_back(0x5f); e.push_back(*s + 0x20); } else { e.push_back(*s); } } return e;}template<typename T> inline const char* ObjName() { const char* s = typeid(T).name(); if constexpr (_IS_WIN) { while (*++s != 0x20); return ++s; } else { while (*s < 0x3a && *s++ != 0x24) {}; return s; }}template<typename T, typename U> inline uint64_t ObjLink() { const char* s = typeid(T).name(), * c = typeid(U).name(); unsigned long long r = 0; if constexpr (_IS_WIN) { while (*++s != 0x20); while (*++c != 0x20); for (; *++s; r *= 0x1f, r += *s - 0x2f); for (; *++c; r *= 0x1f, r += *c - 0x2f); } else { while (*s < 0x3a && *s++ != 0x24) {}; while (*c < 0x3a && *c++ != 0x24) {}; for (; *s; r *= 0x1f, r += *s++ - 0x2f); for (; *c; r *= 0x1f, r += *c++ - 0x2f); } return r;}static std::string toQuotes(const char* s) { std::string e; while (*s) { if (*s == 0x27) { e.push_back(0x27); e.push_back(0x27); } else { e.push_back(*s); } *++s; } return e;}std::ostream& operator<<(std::ostream& os, const tm& _v) {
#ifdef _WIN32
